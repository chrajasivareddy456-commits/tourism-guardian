...
router.post("/register", async (req, res) => {
  try {
    ...
    if (data.role === "authority" && (!env.AUTHORITY_INVITE_CODE || data.authorityCode !== env.AUTHORITY_INVITE_CODE)) return res.status(403).json({ message: "Valid authority invite code required" });
